import { Footer } from "../components/footer";

export default function Home() {
  return (
    <div className="page">
      <p>
        A 501 C3 non-profit organization dedicated to helping families raising
        children or adults with disabilities enjoy happy, healthy memories
        through vacation and recreation experiences.
      </p>
      <br />
      <Footer />
    </div>
  );
}

/*

      <p>
        SMILE Mass strives to become the leading resource for handicap
        accessibility in vacation and recreation experiences. Please help us see
        our vision through. Read letters from some of our supporters right here.
      </p>

      */
